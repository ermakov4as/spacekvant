        ...
        >
          <template v-if="item.type==='dir'">
            ...
            <v-list-item-content>
              ...
              <FilesSpace v-if="openedChildFolders.includes(item.resource_id)" :path="item.path.slice(5)" />
            </v-list-item-content>
...
<script>
import FilesSpace from './FilesSpace'
import FolderName from '@/components/AeroSpace/Modal/FolderName'
import { HTTP_SPACE, HTTP_UPLOAD } from '@/http-common'
...
export default {
  ...
  methods: {
    ...
    getDiskData(_path) {
      ...
      HTTP_SPACE.get(`/disk/resources${query}`)
        .then(response => {
          // TODO: <= 20
    ...
    prepareFileToUpload(event) {
      if (event.target.files[0]) {
        ...
        HTTP_SPACE.get(`/disk/resources/upload${query}`)
          .then((response) => {
    ...
    deleteItem(_path, name, type) {
      ...
      if (confirmDeleting) {
        let query = '?path=' + _path
        HTTP_SPACE.delete(`/disk/resources${query}`)
          .then(response => {
            this.getDiskData(this.path)
            this.$notify({
              group: 'foo',
    ...
    downloadFile(_path) {
      let query = '?path=' + _path
      HTTP_SPACE.get(`/disk/resources/download${query}`)
        .then(response => {
